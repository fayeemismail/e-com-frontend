      ...
      <div className="divide-y divide-[#e8e6e2]">
        {products.map((p) => (
          <div key={p.id} className="flex items-center gap-4 py-4 cursor-pointer group">
            <div className="relative w-16 h-16 shrink-0 bg-[#f0eeea] overflow-hidden">
              <img src={p.image} alt={p.name} className="absolute inset-0 w-full h-full object-cover group-hover:opacity-0 transition-opacity duration-300" />
              <img src={p.hoverImage} alt="" className="absolute inset-0 w-full h-full object-cover opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
            </div>
