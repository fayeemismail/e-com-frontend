  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2">
        <div className="flex">
          {[1,2,3,4,5].map((s) => (
            <svg key={s} width="12" height="12" viewBox="0 0 24 24" fill={s <= Math.round(product.rating) ? "#1a1a1a" : "none"} stroke="#1a1a1a" strokeWidth="1.5">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <button className="flex-1 py-3.5 bg-[#1a1a1a] text-white text-[11px] tracking-[0.18em] uppercase border-none cursor-pointer hover:bg-[#333] transition-colors duration-200">
          Buy — ${product.price}
        </button>
        <button
          onClick={() => setRentOpen((o) => !o)}
          className="flex-1 py-3.5 bg-transparent text-[#1a1a1a] text-[11px] tracking-[0.18em] uppercase border border-[#1a1a1a] cursor-pointer hover:bg-[#1a1a1a] hover:text-white transition-colors duration-200"
        >
          Rent This
        </button>
      </div>
          ...
          {[
            ...
            <div key={opt.period} className="flex items-center justify-between py-2 border-b border-[#f5f5f5] last:border-0">
              ...
              <div className="flex items-center gap-3">
                ...
                <button className="text-[10px] tracking-[0.12em] uppercase text-[#1a1a1a] border border-[#1a1a1a] px-3 py-1 bg-transparent cursor-pointer hover:bg-[#1a1a1a] hover:text-white transition-colors duration-150">
                  Select
                </button>
              </div>
            </div>
